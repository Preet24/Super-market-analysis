...
## Introduction
We will clear the console and import the library. The data is available from this \href{https://www.kaggle.com/aungpyaeap/supermarket-sales}{link}. The data set contains transition details of a supermarket chains in Myanmar, from three branches in three cities Yangon, Naypyitaw and Mandalay for 85 days.\
...
p
```
From the above two graph graph the mode of payment is fairly insensitive to cost, and we can validate our conclusion from the below graph. As a result we can conclude that mode of payment is not a suitable feature to better understand dataset.\
...
p
```
We now see how for the month of `r m`, except for the time between 11:30 AM 12:30 PM when number of female decline otherwise and both male and female visit the super market at the same time. Otherwise the frequency goes increase and decease every other hour.\
```{r,fig.width = 20,fig.height = 6,warning=FALSE,message=FALSE,echo=FALSE}
data=read.csv("D:/ML datasets/unzip files/Supermarket/supermarket_sales - Sheet1.csv")
data$Date=as.Date(data$Date,format="%m/%d/%Y")
data$Time=as.POSIXct(data$Time,format="%H:%M")
...
p
```
Similarly for members and normal customers, we see that no is no correlations between time and membership.\
```{r,fig.width = 20,fig.height = 6,warning=FALSE,message=FALSE,echo=FALSE}
data=read.csv("D:/ML datasets/unzip files/Supermarket/supermarket_sales - Sheet1.csv")
data$Date=as.Date(data$Date,format="%m/%d/%Y")
data$Time=as.POSIXct(data$Time,format="%H:%M")
...
p
```
As we can see from the above graph, there is no correlation between the city and timing of people visiting the super market. We will try to look at the total money spend depends on the  week of the month.As we can wee from the below graph spending increase on the first week of a new month and a decrease on the last day of the month. \
```{r,fig.width = 20,fig.height = 6,warning=FALSE,message=FALSE,echo=FALSE}
data=read.csv("D:/ML datasets/unzip files/Supermarket/supermarket_sales - Sheet1.csv")
...
```
As we can see from the above and below graph, female have a higher 75% quantile and tail except for Yagon where male have a longer tail but still have less 75% quantile then the female for the city of Yagon. Otherwise there is negligible difference on spending on the bases of gender.\
```{r,results='hide',warning=FALSE,message=FALSE,echo=FALSE}
ggplot(data=data,aes(y=Total,x=City,fill=Gender))+geom_violin(draw_quantiles = c(0.25, 0.5, 0.75))
```
As we can see from the below graph, we can say that visually customer type have no effect on the money the customer. We can clearly say that both gender and customer type not usefull to under custer behaviour and segmentation\
```{r,results='hide',warning=FALSE,message=FALSE,echo=FALSE}
data=read.csv("D:/ML datasets/unzip files/Supermarket/supermarket_sales - Sheet1.csv")
